...
describe('JokeService', () => {
  ...
  it('should return only user jokes', function () {
    ...
    expect(jokes.filter(joke => joke.isUserJoke === true)).toEqual(userJokes);
  });

  it('should return next joke', fakeAsync(function () {
    service.nextJoke();
    tick();
    service.currentJoke$.subscribe(joke => {
  ...
  it('should save joke', fakeAsync(function () {
    ...
    tick();
    service.allJokes$.subscribe( allJokes => {
      expect(allJokes[allJokes.length - 1]).toEqual({id: allJokes[allJokes.length - 1].id, isUserJoke: true, category: '123', content: '321'} as JokeInterface);
    });
    service.userJokes$.subscribe(allJokes => {
      expect(allJokes[allJokes.length - 1]).toEqual({id: allJokes[allJokes.length - 1].id, isUserJoke: true, category: '123', content: '321'} as JokeInterface);
    });
  }));
