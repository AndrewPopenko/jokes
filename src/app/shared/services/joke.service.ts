...
export class JokeService {
  ...
  currentJoke$: BehaviorSubject<JokeInterface> = new BehaviorSubject<JokeInterface>({id:'', content: '', category: ''});
  allJokes$: BehaviorSubject<JokeInterface[]> = new BehaviorSubject<JokeInterface[]>([]);
  userJokes$: BehaviorSubject<JokeInterface[]> = new BehaviorSubject<JokeInterface[]>([]);
  ...
  getUserJokes(): JokeInterface[] {
    return this.jokes.filter(item => item.isUserJoke);
  }

  nextJoke() {
    if (this.currentJokeIndex >= this.jokes.length - 1) {
      this.currentJokeIndex = -1;
    }

    this.currentJoke$.next(this.jokes[++this.currentJokeIndex]);
  }
  ...
  deleteJokeById(id: string = ''): void {
    const jokeIndex = this.jokes.findIndex(joke => joke.id === id);
    this.jokes.splice(jokeIndex, 1);
    this.updateObservables();
  }

  private updateObservables() {
    this.allJokes$.next(this.jokes.slice());
