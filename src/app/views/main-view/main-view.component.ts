...
export class MainViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.notifyService.dlgContext$.subscribe(content => this.showDlg = content.showDlg);
  }

  add(): void {
    const dialogRef: MatDialogRef<NewJokeDlgComponent> = this.dialog.open(NewJokeDlgComponent, { data: '' });
    dialogRef.afterClosed().subscribe(result => {
      if (result !== 'closed') {
        this.notifyService.dlgContext$.next({ showDlg: true, text: 'Żart został pomyślnie dodany.', title: 'Sukces' });
      }
    })
  }
